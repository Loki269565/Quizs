...
and writes data/manifest.json describing every test file found, plus every
"notes" file (PDF or Markdown) found alongside them.
...
def list_note_files(dir_path):
    if not os.path.isdir(dir_path):
        return []
    return sorted(f for f in os.listdir(dir_path) if is_note_file(f) and os.path.isfile(os.path.join(dir_path, f)))


def main():
    manifest = {"uncategorized": [], "uncategorized_notes": [], "categories": []}
    ...
    manifest["uncategorized_notes"] = list_note_files(DATA_DIR)
    ...
    for name in top_entries:
        ...
        category = {
            ...
            "notes": list_note_files(cat_path),
            "subfolders": [],
        }
        ...
        for sub_name in sorted(os.listdir(cat_path)):
            ...
            category["subfolders"].append({
                ...
                "notes": list_note_files(sub_path),
            })
